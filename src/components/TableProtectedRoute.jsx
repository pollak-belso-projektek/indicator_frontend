...
export default function TableProtectedRoute({ children, tableName = null }) {
  ...
  if (
    targetTableName === "data-import" ||
    location.pathname === "/adat-import"
  ) {
    const isAdmin = userPermissions?.isAdmin || userPermissions?.isSuperadmin;
    const hasDataTableAccess =
      tableAccess &&
      Array.isArray(tableAccess) &&
  ...
  if (targetTableName === "user") {
    ...
    if (!isSuperadmin && !hasUserTableAccess) {
      console.warn(
        `Access denied to users - superadmin permissions or user table access required for route: ${location.pathname}`
      );
      ...
      return (
        <Navigate
          ...
          state={{
            redirectReason: "insufficient_permissions",
            fromRoute: location.pathname,
            requiredRole: "superadmin_or_user_access",
          }}
          replace
        />
      );
