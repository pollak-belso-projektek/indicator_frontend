...
export default function EgyOktatoraJutoOsszDiak() {
  ...
  const [totalWeeklyHoursPerTeacher, setTotalWeeklyHoursPerTeacher] = useState(22);
  ...
  const [formData, setFormData] = useState({
    ...
    fenntartoAltalEngedelyezettHetiOratulojJogviszony: schoolYears.reduce((acc, year) => {
      acc[year] = 0;
      return acc;
    }, {}),
    
    fenntartoAltalEngedelyezettHetiOraFelnottkepzesi: schoolYears.reduce((acc, year) => {
      acc[year] = 0;
      return acc;
    }, {}),
  });
  ...
  const calculateTotals = useMemo(() => {
    ...
    schoolYears.forEach(year => {
      ...
      const tanuloiOratmeg = formData.fenntartoAltalEngedelyezettHetiOratulojJogviszony[year] || 0;
      const felnottkepzesiOratmeg = formData.fenntartoAltalEngedelyezettHetiOraFelnottkepzesi[year] || 0;
      const szamitottOktatoiLetszam = Math.ceil((tanuloiOratmeg + felnottkepzesiOratmeg) / totalWeeklyHoursPerTeacher) || 0;
      ...
      const felnottkepzesiTotal = formData.felnottkepzesiJogviszonyuTanulok[year] || 0;
      const szakmaiOktatasOsszletszam = tanuloiTotal + felnottkepzesiTotal;
      
      // 3. Calculate egy oktatóra jutó tanulók száma
      const egyOktatoraJutoTanulok = szamitottOktatoiLetszam > 0 
      ...
      <Fade in={true} timeout={800}>
        <Box sx={{ minHeight: 'calc(100vh - 120px)' }}>
            ...
            <CardContent sx={{ p: 2 }}>
              <Stack direction="row" alignItems="center" spacing={2} sx={{ mb: 2 }}>
                ...
                <Typography variant="h4" component="h1" sx={{ fontWeight: 600 }}>
                 27. A szakképző intézményben egy oktatóra jutó (tanulói jogviszonyú + felnőttképzési jogviszonyú) tanulók száma
                </Typography>
              </Stack>
            
              <Typography variant="body1" sx={{ opacity: 0.8 }}>
                Az oktató-tanuló arány nyomon követése a különböző jogviszonyok szerint bontva, automatikus számításokkal.
              </Typography>
            </CardContent>
          </Card>
          ...
          >
              ...
              */}
                <Box sx={{ p: 2, textAlign: 'left' }}>
                    <TextField
                        ...
                        onChange={(e) => setTotalWeeklyHoursPerTeacher(parseFloat(e.target.value) || 22)}
              ...
              <Box sx={{ p: 3, bgcolor: '#fafbfc', minHeight: '400px' }}>
                  ...
                  >
                    <CardContent sx={{ p: 3 }}>
                      <Stack direction="row" alignItems="center" spacing={2} sx={{ mb: 3 }}>
                        ...
                        <Box>
                          ...
                          <Typography variant="body2" sx={{ color: 'text.secondary', fontStyle: 'italic' }}>
                            Automatikusan számított: tanulók összlétszáma ÷ számított oktatói létszám
                          </Typography>
                        </Box>
                      </Stack>
                      <TableContainer component={Paper} elevation={3}>
                        <Table>
                          <TableHead>
                            <TableRow sx={{ background: 'linear-gradient(45deg, #e8f5e8, #c8e6c9)' }}>
                              <TableCell sx={{ fontWeight: 700, fontSize: '1rem' }}>Végeredmény</TableCell>
                              {schoolYears.map(year => (
                                <TableCell key={year} align="center" sx={{ fontWeight: 700, fontSize: '1rem' }}>
                                  {year}
                                </TableCell>
                              ))}
                            </TableRow>
                          ...
                          <TableBody>
                            <TableRow sx={{ background: 'linear-gradient(45deg, #4caf5040, #66bb6a40)' }}>
                              <TableCell sx={{ fontWeight: 700, fontSize: '1rem' }}>Tanulók/Oktató</TableCell>
                              {schoolYears.map(year => (
                                <TableCell 
                                  key={year} 
                                  align="center" 
                                ...
                                >
                                  {calculateTotals[year]?.egyOktatoraJutoTanulok || 0}
                                </TableCell>
                              ))}
                            </TableRow>
                          </TableBody>
                ...
                <Grid item xs={12} md={6}>
                  <Card elevation={3} sx={{ border: '2px solid #ff9800', bgcolor: '#fff3e0', height: '100%' }}>
                    <CardContent>
                      <Stack direction="row" alignItems="center" spacing={2} sx={{ mb: 2 }}>
                        <FunctionsIcon sx={{ color: '#ff9800', fontSize: 28 }} />
                      ...
                      <TableContainer component={Paper} elevation={2}>
                        <Table size="small">
                          <TableHead>
                            <TableRow sx={{ background: 'linear-gradient(45deg, #fff3e0, #ffe0b2)' }}>
                              {schoolYears.map(year => (
                            ...
                            <TableRow>
                              {schoolYears.map(year => (
                                <TableCell 
                                  key={year} 
                                  align="center" 
                                  sx={{ fontWeight: 700, color: '#ff9800', fontSize: '1.1rem' }}
                                >
                                  {calculateTotals[year]?.szamitottOktatoiLetszam || 0}
                                </TableCell>
                              ))}
                            </TableRow>
                          </TableBody>
                ...
                <Grid item xs={12} md={6}>
                  <Card elevation={3} sx={{ border: '2px solid #2196f3', bgcolor: '#e3f2fd', height: '100%' }}>
                    <CardContent>
                      <Stack direction="row" alignItems="center" spacing={2} sx={{ mb: 2 }}>
                      ...
                      <TableContainer component={Paper} elevation={2}>
                        <Table size="small">
                          <TableHead>
                            <TableRow sx={{ background: 'linear-gradient(45deg, #e3f2fd, #bbdefb)' }}>
                              {schoolYears.map(year => (
                                <TableCell key={year} align="center" sx={{ fontWeight: 600 }}>
                                  {year}
                                </TableCell>
                            ...
                            <TableRow>
                              {schoolYears.map(year => (
                                <TableCell 
                                  key={year} 
                                  align="center" 
                                  sx={{ fontWeight: 700, color: '#2196f3', fontSize: '1.1rem' }}
                                >
                                  {calculateTotals[year]?.szakmaiOktatasOsszletszam || 0}
                                </TableCell>
                              ))}
                            </TableRow>
                          </TableBody>
            ...
            <TabPanel value={activeTab} index={1}>
              <Box sx={{ p: 3, bgcolor: '#fafbfc', minHeight: '400px' }}>
                            ...
                            <TableRow>
                              {schoolYears.map(year => (
                                <TableCell key={year} align="center" sx={{ py: 1 }}>
                                  <TextField
                                    type="number"
                                    value={formData.tanuloiJogviszonyuTanulok[year]}
                                    onChange={(e) => handleInputChange('tanuloiJogviszonyuTanulok', year, e.target.value)}
                                    size="small"
                                    inputProps={{ 
                                      min: 0, 
                ...
                <Grid item xs={12} md={6}>
                  <Card elevation={2} sx={{ height: '100%', bgcolor: '#e8f5e8' }}>
                    <CardContent>
                      <Stack direction="row" alignItems="center" spacing={2} sx={{ mb: 2 }}>
                            ...
                            <TableRow>
                              {schoolYears.map(year => (
                                <TableCell key={year} align="center" sx={{ py: 1 }}>
                                  <TextField
                                    type="number"
                                    value={formData.felnottkepzesiJogviszonyuTanulok[year]}
                                    onChange={(e) => handleInputChange('felnottkepzesiJogviszonyuTanulok', year, e.target.value)}
                                    size="small"
                                    inputProps={{ 
                                      min: 0, 
                                      style: { textAlign: 'center', fontWeight: 600 } 
                  ...
                  <Card elevation={2} sx={{ height: '100%', bgcolor: '#f3e5f5' }} >
                    <CardContent>
                      ...
                      <TableContainer component={Paper} elevation={2}>
                        <Table size="small">
                          <TableHead>
                            <TableRow sx={{ background: 'linear-gradient(45deg, #f3e5f5, #e1bee7)' }}>
                              {schoolYears.map(year => (
                                <TableCell key={year} align="center" sx={{ fontWeight: 600 }}>
                                  {year}
                                </TableCell>
                            ...
                            <TableRow>
                              {schoolYears.map(year => (
                                <TableCell key={year} align="center" sx={{ py: 1 }}>
                                  <TextField
                                    type="number"
                                    value={formData.fenntartoAltalEngedelyezettHetiOratulojJogviszony[year]}
                                    onChange={(e) => handleInputChange('fenntartoAltalEngedelyezettHetiOratulojJogviszony', year, e.target.value)}
                                    size="small"
                                    inputProps={{ 
                                      min: 0, 
                                      style: { textAlign: 'center', fontWeight: 600 } 
                  ...
                  <Card elevation={2} sx={{ height: '100%', bgcolor: '#fff3e0' }}>
                    <CardContent>
                      ...
                      <TableContainer component={Paper} elevation={2}>
                        <Table size="small">
                          <TableHead>
                            <TableRow sx={{ background: 'linear-gradient(45deg, #fff3e0, #ffcc02)' }}>
                              {schoolYears.map(year => (
                                <TableCell key={year} align="center" sx={{ fontWeight: 600 }}>
                                  {year}
                                </TableCell>
                            ...
                            <TableRow>
                              {schoolYears.map(year => (
                                <TableCell key={year} align="center" sx={{ py: 1 }}>
                                  <TextField
                                    type="number"
                                    value={formData.fenntartoAltalEngedelyezettHetiOraFelnottkepzesi[year]}
                                    onChange={(e) => handleInputChange('fenntartoAltalEngedelyezettHetiOraFelnottkepzesi', year, e.target.value)}
                                    size="small"
                                    inputProps={{ 
                                      min: 0, 
                                      style: { textAlign: 'center', fontWeight: 600 } 
          ...
          <Snackbar
            ...
            sx={{ 
              '& .MuiSnackbar-root': {
                transform: notification.open ? 'translateY(0)' : 'translateY(100%)',
